...
$nome = $email = $senha = $confirm_senha = $telefone = $cpf = $rua = $numero = $cidade = "";
$nome_err = $email_err = $senha_err = $confirm_senha_err = $telefone_err = $cpf_err = $rua_err = $numero_err = $cidade_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nome_err) && empty($email_err) && empty($senha_err) && empty($confirm_senha_err) && empty($telefone_err) && empty($cpf_err) && empty($rua_err) && empty($numero_err) && empty($cidade_err)) {

        $sql = "INSERT INTO usuarios (nome, email, senha, telefone, cpf, rua, numero, cidade, tipo_usuario) VALUES (:nome, :email, :senha, :telefone, :cpf, :rua, :numero, :cidade, 'comum')";
...
<body>
    <div class="container">
        <h2>Registro</h2>
        <p>Sou veterinário <a href="registervet.php">Cadastre-se aqui</a>.</p>
